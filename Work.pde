...
class Work {
  ...
  private color[] getGridSubPalette(int position) {
    ...
    switch(position) {
    case 0: 
      out = new color[]{palette[0], palette[2], palette[0], palette[3], palette[11], palette[12], palette[0], palette[0]};
      break;
    case 1:
      out = new color[]{palette[0], palette[1], palette[2], palette[3], palette[0], palette[1], palette[2], palette[3]};
      break;
    case 2: 
      out = new color[]{palette[3], palette[2], palette[0], palette[1], palette[11], palette[1], palette[12], palette[0]};
      break;
    case 5: 
      out = new color[]{palette[1], palette[5], palette[7], palette[0], palette[8], palette[10], palette[0], palette[1]};
      break;
    case 7: 
      out = new color[]{palette[7], palette[5], palette[0], palette[0], palette[8], palette[0], palette[0], palette[10]};
      break;
    default:
      out = new color[]{Palette.WHITE, Palette.WHITE, Palette.WHITE, Palette.WHITE, Palette.WHITE, Palette.WHITE, Palette.WHITE, Palette.WHITE};
    }
  ...
  private void genGrids(float size) {
    grid[0] = new Grid(0, 0, size, true, palette[0], getGridSubPalette(0));//
    grid[1] = new Grid(3*size, 0, size, false, palette[0], getGridSubPalette(1));
    grid[2] = new Grid(size, size, size, true, palette[0], getGridSubPalette(2));//
    grid[3] = new Grid(2*size, size, size, false, palette[0], getGridSubPalette(3));
    grid[4] = new Grid(size, 2*size, size, false, palette[0], getGridSubPalette(4));
    grid[5] = new Grid(2*size, 2*size, size, true, palette[0], getGridSubPalette(5));//
    grid[6] = new Grid(0, 3*size, size, false, palette[0], getGridSubPalette(6));
    grid[7] = new Grid(3*size, 3*size, size, true, palette[0], getGridSubPalette(7));//
  }
